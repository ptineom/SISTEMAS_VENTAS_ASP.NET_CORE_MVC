...
var oMetodos = {
    ...
    showLoading: function (idElementoContenedor) {
        if (idElementoContenedor == null || idElementoContenedor == "" || idElementoContenedor == undefined) {
            var block = $("<div id='dvLoading'></div>");
            $("body").prepend(block);
        } else {
            var block = $("<div id='dvLoading-partial'></div>");
            var modalContent = $("#" + idElementoContenedor).find(".modal-content");
            $("#" + idElementoContenedor).find(".modal-body").prepend(block);
        }
    },
    hideLoading: function () {
        ...
        var dvLoadingpartial = $("#dvLoading-partial");
        if (dvLoadingpartial.length > 0) {
            dvLoadingpartial.remove();
        }
    },
    ajax: function (modelo) {
        //Predeterminados.
        let contentType = "application/x-www-form-urlencoded; charset=UTF-8";
        let processData = true;
        let dataType = "json";

        if (modelo.dataType != undefined) dataType = modelo.dataType;
        if (modelo.contentType != undefined) contentType = modelo.contentType;
        if (modelo.bSaveFile != undefined) {
            if (modelo.bSaveFile) {
                contentType = false;
                processData = false;
            }
        ...
        $.ajax({
            ...
            success: modelo.success,
            error: function (xhr) {
                if (modelo.callbackError != undefined) {
                    modelo.callbackError();
                }
                if (xhr.responseJSON != undefined) {
                    oAlerta.mensajeBootbox(xhr.responseJSON.message != undefined ? xhr.responseJSON.message : xhr.responseJSON.Message, "danger");
                } else {
                    oAlerta.mensajeBootbox("Error: " + xhr.statusText, "warning");
                }
            },
            cache: false,
            beforeSend: function (xhr) {
                if (modelo.jwtToken != undefined) {
                    if (modelo.jwtToken != "") {
                        let jwtDecode = oMetodos.parseJwt(modelo.jwtToken);
                        //La fecha de expiración está en "tiempo unix"
                        //el backend lo genera con fecha UTC en fomato "tiempo unix", pero en javascript lo muestra con fecha local hay que formtearlo en utc.
                        let expiracion = jwtDecode.exp;
                        var expiracionUtc = moment.unix(expiracion).utc().format();
                        var actualUtc = moment.utc().format();
                        if (expiracionUtc < actualUtc) {
                            if (modelo.callbackError != undefined) {
                                modelo.callbackError();
                            }
                            //throw new Error("El token ha expirado.");
                            oAlerta.mensajeError("El token ha expirado", "danger");
                            xhr.abort();
                        }
                        xhr.setRequestHeader('Authorization', 'Bearer ' + modelo.accessToken);
                    }
                }
                oMetodos.showLoading(modelo.nameElementContainer);
            },
            complete: function () {
                oMetodos.hideLoading();
            }
        })
    },
    ...
    formatoTblListaOld: function (bBotonNuevo, arrColumns, bFilter, bPaginate, bInfo, bAutoWidth, fnCallBack, fnNuevo, classBtn, textBtn) {
        var formatoBotonNuevo = [];
        if (bBotonNuevo) {
            let text = '<span style="color:white"><i class="fas fa-file" ></i> Nuevo</span>';
            let className = 'btn btn-primary';
            if (classBtn != undefined) {
                className = classBtn;
            }
    ...
    inicializarDateRangePicker: function (selectores$, flgRango) {
        let range = undefined
        if (flgRango) {
            range = {
                'Hoy': [moment(), moment()],
    ...
    inicializarDatePicker: function (selectores$) {
        let selector = selectores$ == undefined ? '.date-picker' : selectores$;
        $(selector).datepicker({
            autoclose: true,
            todayHighlight: true,
    ...
    inicializarSpinner: function (flgAlPorcentaje) {
        flgAlPorcentaje = flgAlPorcentaje = undefined ? false : flgAlPorcentaje;
        $('.spinner .btn:first-of-type').on('click', function () {
            let input = $('.spinner input')[0];
            let value = input.value == "" ? 0 : parseInt(input.value);
            let dataSalto = input.getAttribute("data-salto");
            let dataEsPorcentaje = input.getAttribute("data-EsPorcentaje");
            dataSalto = parseInt(dataSalto != null ? (isNaN(dataSalto) ? 1 : dataSalto) : 1);
            dataEsPorcentaje = dataEsPorcentaje != null ? dataEsPorcentaje : false;
            let aumentar = (value + dataSalto);
            if (dataEsPorcentaje) {
                if (aumentar <= 100) {
